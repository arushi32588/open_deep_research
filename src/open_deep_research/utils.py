...
@traceable
async def tavily_search_async(query_list: list[str], **kwargs) -> list[dict]:
    """Execute Tavily search asynchronously for a list of queries.
    
    Args:
        query_list: List of search queries to execute
        **kwargs: Additional parameters to pass to the Tavily API
    ...
    for query in query_list:
        try:
            # Ensure the query is properly encoded as UTF-8
            try:
                # Try to encode and decode to ensure proper UTF-8 handling
                query = query.encode('utf-8').decode('utf-8')
            except UnicodeEncodeError:
                # If encoding fails, replace non-ASCII characters
                query = query.encode('ascii', 'replace').decode('ascii')
            
            # Execute the search
            response = await client.search(query, **kwargs)
            
            # Process the response to ensure proper encoding
            if isinstance(response, dict):
                # Process content and raw_content fields if they exist
                if "results" in response:
                    for result in response["results"]:
                        if "content" in result:
                            try:
                                result["content"] = result["content"].encode('utf-8').decode('utf-8')
                            except (UnicodeEncodeError, UnicodeDecodeError):
                                result["content"] = result["content"].encode('ascii', 'replace').decode('ascii')
                        
                        if "raw_content" in result:
                            try:
                                result["raw_content"] = result["raw_content"].encode('utf-8').decode('utf-8')
                            except (UnicodeEncodeError, UnicodeDecodeError):
                                result["raw_content"] = result["raw_content"].encode('ascii', 'replace').decode('ascii')
                
                results.append(response)
            else:
                # If response is not a dictionary, create an empty result
                results.append({
                    "query": query,
            ...
            print(f"Error in tavily_search_async: {str(e)}")
...
async def select_and_execute_search(
    ...
    topic: str,
    params_to_pass: dict,
    local_documents: Optional[dict] = None,
    **kwargs
) -> List[Dict[str, Any]]:
    """Simplified search function that only uses local documents"""
    print("DEBUG: Using simplified search with local documents only")
    ...
    try:
        ...
        vector_store_dir = "vector_store"  # Default directory
        if local_documents and isinstance(local_documents, dict):
            vector_store_dir = local_documents.get("vector_store_dir", vector_store_dir)
        
        vector_store.load(vector_store_dir)
        print(f"DEBUG: Loaded vector store from {vector_store_dir}")
    except Exception as e:
        print(f"DEBUG: Error loading vector store: {str(e)}")
        return []
    
    # Initialize hybrid retrieval system with default values
    print("DEBUG: Initializing hybrid retrieval system")
    reranker_model = "gpt-3.5-turbo"
    max_results = 10
    
    if local_documents and isinstance(local_documents, dict):
        reranker_model = local_documents.get("reranker_model", reranker_model)
        max_results = local_documents.get("max_results", max_results)
    ...
    print("DEBUG: Performing hybrid search for queries")
    search_queries = [
        f"{topic} market capitalization revenue growth rate 2023 statistics",
        f"{topic} revenue breakdown hardware products services Q4 2023",
        f"{topic} revenue breakdown Q4 2023 e-commerce marketplace cloud services percentage",
        f"{topic} cloud services revenue growth compared to retail platform financial analysis",
        f"{topic} services revenue growth subscription ecosystem analysis",
        f"{topic} market share dominance comparison",
        f"{topic} business model strategy analysis",
        f"{topic} competitive advantages challenges future outlook",
        f"{topic} growth trajectory financial performance revenue streams",
        f"{topic} ecosystem comparison hardware software services integration"
    ]
    ...
    for query in search_queries:
        ...
        try:
            ...
            results = await hybrid_system.search(query)
            if results:
    ...
    # Format results
    print("DEBUG: Formatting hybrid results")
    formatted_results = []
    for result in all_results[:20]:  # Limit to top 20 results
        try:
            # Handle Document objects properly
            formatted_results.append({
                "title": getattr(result, "title", "Local Document"),
                "content": getattr(result, "content", ""),
                "url": getattr(result, "source", "local_document"),
                "score": getattr(result, "score", 0.0)
            })
        except Exception as e:
            print(f"DEBUG: Error formatting result: {str(e)}")
            continue
